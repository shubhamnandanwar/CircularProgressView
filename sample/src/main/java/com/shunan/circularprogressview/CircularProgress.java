...
public final class CircularProgress extends View {
    ...
    public CircularProgress(@NonNull final Context context) {
        super(context);
        initialize(context, null, 0, 0);

    }

    public CircularProgress(@NonNull final Context context, @Nullable final AttributeSet attrs) {
        super(context, attrs);
        initialize(context, attrs, 0, 0);
    }

    public CircularProgress(@NonNull final Context context, @Nullable final AttributeSet attrs, final int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initialize(context, attrs, defStyleAttr, 0);
    }

    @RequiresApi(Build.VERSION_CODES.LOLLIPOP)
    public CircularProgress(@NonNull final Context context, @Nullable final AttributeSet attrs, final int defStyleAttr, final int defStyleRes) {
    ...
    private void initialize(@NonNull final Context context, @Nullable final AttributeSet attributeSet,
        ...
        /*else {
            ...
            try {
                ...
                progress = attributes.getFloat(R.styleable.CircularProgressBar_progress, DEFAULT_PROGRESS);
                final float startAngle = attributes.getFloat(R.styleable.CircularProgressBar_startAngle, DEFAULT_START_ANGLE);
                checkStartAngle(startAngle);
                startAngle = startAngle;
                final float minimumAngle = attributes.getFloat(R.styleable.CircularProgressBar_indeterminateMinimumAngle, DEFAULT_INDETERMINATE_MINIMUM_ANGLE);
                checkIndeterminateMinimumAngle(minimumAngle);
                mIndeterminateMinimumAngle = minimumAngle;
                final long progressDuration = attributes.getInteger(R.styleable.CircularProgressBar_progressAnimationDuration, DEFAULT_PROGRESS_ANIMATION_DURATION);
                checkAnimationDuration(progressDuration);
                mProgressAnimator.setDuration(progressDuration);
                final long rotationDuration = attributes.getInteger(R.styleable.CircularProgressBar_indeterminateRotationAnimationDuration, DEFAULT_INDETERMINATE_ROTATION_ANIMATION_DURATION);
                checkAnimationDuration(rotationDuration);
                mIndeterminateStartAnimator.setDuration(rotationDuration);
                final long sweepDuration = attributes.getInteger(R.styleable.CircularProgressBar_indeterminateSweepAnimationDuration, DEFAULT_INDETERMINATE_SWEEP_ANIMATION_DURATION);
                checkAnimationDuration(sweepDuration);
                mIndeterminateSweepAnimator.setDuration(sweepDuration);
                mForegroundStrokePaint.setColor(attributes.getColor(R.styleable.CircularProgressBar_foregroundStrokeColor, DEFAULT_FOREGROUND_STROKE_COLOR));
                mBackgroundStrokePaint.setColor(attributes.getColor(R.styleable.CircularProgressBar_backgroundStrokeColor, DEFAULT_BACKGROUND_STROKE_COLOR));
                final float foregroundWidth = attributes.getDimension(R.styleable.CircularProgressBar_foregroundStrokeWidth, Math.round(DEFAULT_FOREGROUND_STROKE_WIDTH_DP * displayMetrics.density));
                checkWidth(foregroundWidth);
                mForegroundStrokePaint.setStrokeWidth(foregroundWidth);
                mForegroundStrokePaint.setStrokeCap(getStrokeCap(attributes.getInt(R.styleable.CircularProgressBar_foregroundStrokeCap, DEFAULT_FOREGROUND_STROKE_CAP)));
                final float backgroundWidth = attributes.getDimension(R.styleable.CircularProgressBar_backgroundStrokeWidth, Math.round(DEFAULT_BACKGROUND_STROKE_WIDTH_DP * displayMetrics.density));
                checkWidth(backgroundWidth);
                mBackgroundStrokePaint.setStrokeWidth(backgroundWidth);
                mAnimateProgress = attributes.getBoolean(R.styleable.CircularProgressBar_animateProgress, DEFAULT_ANIMATE_PROGRESS);
                mDrawBackgroundStroke = attributes.getBoolean(R.styleable.CircularProgressBar_drawBackgroundStroke, DEFAULT_DRAW_BACKGROUND_STROKE);
                mIndeterminate = attributes.getBoolean(R.styleable.CircularProgressBar_indeterminate, DEFAULT_INDETERMINATE);
            } finally {
                if (attributes != null) {
                    attributes.recycle();
                }
